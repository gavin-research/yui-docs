...
// par/2 
        ...
        for (let j = 0; j < nivel.length; j +=1){
            ...
            if (mapeado[i][j] % 2 !== 0){
                ...
                console.log('REEE', mapeado[i][j]);
                console.log('Hash for entry ', levelhash, orderhash);
                console.log(finalHash);

                if (levelhash < mapeado.length){
                    leafHashes[levelhash].splice(orderhash-1, 0, finalHash);
                    console.log(leafHashes[levelhash], 'Bueno');
                    mapeado[levelhash].splice(orderhash-1, 0, orderhash);
                }
